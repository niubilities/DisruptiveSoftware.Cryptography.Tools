    ...
    internal static class Program
    {
            ...
            IEnumerable<string>? subjectAlternativeNames)
        {
            var subjectAlternativeNamesExtension =
                new DerSequence(
                    (subjectAlternativeNames ?? Array.Empty<string>()).Select(name => new GeneralName(GeneralName.DnsName, name))
        ...
        private static X509Certificate2 CreateCertificateAuthorityCertificate(string subjectName,
            ...
            KeyPurposeID[]? usages)
        {
            ...
            if (usages != null && usages.Any()) AddExtendedKeyUsage(certificateGenerator, usages);
        ...
        private static X509Certificate2 IssueCertificate(string subjectName, X509Certificate2 issuerCertificate,
            string[]? subjectAlternativeNames, KeyPurposeID[]? usages)
        {
            // It's self-signed, so these are the same.
        ...
        private static int Main(string[] args)
        {
            ...
            switch (mode.ToLower())
            {
                case "self":
                {
                    if (args.Length != 3) return ShowUsage();
                    ...
                    var certificate = CreateSelfSignedCertificate(
                        subjectName,
                        new[] { "server", "server.mydomain.com" },
                ...
                case "issue":
                {
                    if (args.Length != 4) return ShowUsage();
                    ...
                    var certificate = IssueCertificate(
                        subjectName,
                        issuerCertificate,
                        new[] { "server", "server.mydomain.com" },
                        new[] { KeyPurposeID.IdKPServerAuth });
