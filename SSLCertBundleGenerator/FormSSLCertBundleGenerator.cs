...
namespace SSLCertBundleGenerator
{
  ...
  public partial class FormSslCertBundleGenerator : Form
  {
    ...
    private void ButtonBrowse_Click(object? sender, EventArgs e)
    {
      ...
      if (DialogResult.OK == folderBrowserDialog.ShowDialog()) textBoxSavePath.Text = folderBrowserDialog.SelectedPath;
    }
    ...
    private async void GenerateCertificatesAsync()
    {
      try
      {
        ...
        if (Directory.GetFiles(savePath).Length > 0) throw new Exception("Destination directory must be empty.");
        ...
        await Task.Run(
          () =>
          {
            ...
            if (string.IsNullOrWhiteSpace(textBoxPassword.Text)) password = textBoxPassword.Text.ToSecureString();
